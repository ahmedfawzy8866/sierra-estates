...
export default function LandingPage() {
  ...
  const [activeTab, setActiveTab] = useState<'explore'|'careers'>('explore');
    ...
    <div style={{ minHeight: '100vh', background: th.bg, color: th.text, transition: 'background .5s, color .5s' }} dir={T.dir}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 300, height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 48px', background: scrolled ? th.navBg : 'transparent', backdropFilter: scrolled ? 'blur(20px)' : 'none', borderBottom: scrolled ? `1px solid ${th.border}` : '1px solid transparent', transition: 'all .4s cubic-bezier(.16,1,.3,1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
          <div onClick={() => setActiveTab("explore")}><ShieldLogo size={38} /></div>
          <div>
...
{/* ══ SIERRA AI ══ */}
      ...
      <>
      {/* ══ CAREERS TAB ══ */}
      <section style={{ paddingTop: 140, paddingBottom: 96, minHeight: '80vh', background: th.heroBg }}>
        <div style={{ ...sec, maxWidth: 800 }}>
          <div className="reveal text-center mb-12">
            <Briefcase size={40} color={G} className="mx-auto mb-4" />
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(32px, 4.5vw, 56px)', fontWeight: 300, color: th.text, marginBottom: 16 }}>
              {isAr ? 'انضم إلى فريق النخبة' : 'Join the Elite Team'}
            </h1>
            <p style={{ fontSize: 15, fontWeight: 300, lineHeight: 1.8, color: th.textSub, fontFamily: isAr ? "'Cairo', sans-serif" : "'Jost', sans-serif" }}>
          ...
          <div className="reveal p-8 rounded-3xl" style={{ background: th.card, border: `1px solid ${th.border}`, boxShadow: '0 16px 40px rgba(0,0,0,0.15)' }}>
            <form className="flex flex-col gap-6" onSubmit={(e) => { e.preventDefault(); alert(isAr ? 'تم الاستلام' : 'Application received'); }}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '.14em', color: th.textMuted, textTransform: 'uppercase', marginBottom: 8, fontFamily: "'Jost', sans-serif" }}>{isAr ? 'الاسم الكامل' : 'Full Name'}</label>
                  <input type="text" required style={{ width: '100%', background: th.bg, border: `1px solid ${th.border}`, borderRadius: 8, padding: '14px 18px', color: th.text, outline: 'none' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '.14em', color: th.textMuted, textTransform: 'uppercase', marginBottom: 8, fontFamily: "'Jost', sans-serif" }}>{isAr ? 'رقم الهاتف' : 'Phone Number'}</label>
                  <input type="tel" required style={{ width: '100%', background: th.bg, border: `1px solid ${th.border}`, borderRadius: 8, padding: '14px 18px', color: th.text, outline: 'none' }} />
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '.14em', color: th.textMuted, textTransform: 'uppercase', marginBottom: 8, fontFamily: "'Jost', sans-serif" }}>{isAr ? 'رابط السيرة الذاتية أو لينكد إن' : 'LinkedIn or Resume Link'}</label>
                <input type="url" required style={{ width: '100%', background: th.bg, border: `1px solid ${th.border}`, borderRadius: 8, padding: '14px 18px', color: th.text, outline: 'none' }} />
              </div>
              <button type="submit" style={{ width: '100%', background: `linear-gradient(135deg,${G2},${G})`, color: '#071422', border: 'none', cursor: 'pointer', fontFamily: "'Jost', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', padding: '16px', borderRadius: 8, marginTop: 8 }}>
